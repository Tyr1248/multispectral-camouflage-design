...
from color_calculate.colors import utils
...
def spectrum_to_XYZ(
    ...
    assert np.all(cmfs[:, 0] == wavelengths)
    assert np.all(wavelengths == cmfs[:, 0])
    assert np.all(wavelengths == illuminant[:, 0])
    ...
    denominator = np.sum(((Iy[1:] + Iy[:-1]) / 2) * delta_wavelengths)

    numerator_X = np.sum(((SIx[1:] + SIx[:-1]) / 2) * delta_wavelengths)
    numerator_Y = np.sum(((SIy[1:] + SIy[:-1]) / 2) * delta_wavelengths)
    numerator_Z = np.sum(((SIz[1:] + SIz[:-1]) / 2) * delta_wavelengths)
