...
def interpolate_wl_numpy(wls_values, wavelengths):
    ...
    Args:
        wls_values: NumPy array of shape (N, 2); column 0 holds wavelengths,
            column 2 holds the corresponding values.
        wavelengths: 1-D NumPy array of target wavelength points.

    Returns:
        values_interpolated: values interpolated at the target wavelengths.
    """
    ...
    # Check whether the target wavelengths lie within the data range
    min_wl = np.min(wls)
    max_wl = np.max(wls)
...
def interpolate_material_n_k_by_wl_numpy(material, wavelengths):
    """
    Get the refractive index (n) and extinction coefficient (k) of a material
    as a function of wavelength (NumPy implementation).

    Args:
        material: string, material name.
        wavelengths: 1-D NumPy array of wavelength points at which to compute
    ...
    if material == "Air":
        ...
        # Load wavelength data directly; no unit conversion needed
...
def get_n_k(materials, wl):
    """
    Get the complex refractive index (n + ik) of a multilayer material stack
    as a function of wavelength (NumPy implementation).
    ...
    Returns:
        n_k: complex array of shape (num_wl, num_layers) with the complex
            refractive index of each layer at each wavelength point.
    """
